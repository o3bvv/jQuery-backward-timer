...
(function($) {
    ...
    $.backward_timer = function(element) {
        ...
        plugin.methods = {
            ...
            , _render_seconds_to_wrapper: function() {
                var dhms = plugin.methods._seconds_to_dhms(plugin.seconds_left)
                  , value = plugin.settings.format
                  , $days = "d%"
                  , $hours = "h%"
                ...
                if (value.indexOf("d%") !== -1) {
                    $days  = $days.replace('d%', dhms.d)
                    $hours = $hours.replace('h%', plugin.methods._check_leading_zero(dhms.h))
                    value  = value
                            .replace('d%', dhms.d)
                            .replace('h%', plugin.methods._check_leading_zero(dhms.h))
                } else {
                    $hours = $hours.replace('h%', dhms.d * 24 + dhms.h)
                    value = value.replace('h%', dhms.d * 24 + dhms.h)
                }


                $minutes = $minutes.replace('m%', plugin.methods._check_leading_zero(dhms.m))
                $seconds = $seconds.replace('s%', plugin.methods._check_leading_zero(dhms.s))
                value = value
                        .replace('m%', plugin.methods._check_leading_zero(dhms.m))
                        .replace('s%', plugin.methods._check_leading_zero(dhms.s))
                ...
                if ( plugin.settings.use_wrapper ) {
                    if ( undefined != plugin.settings.wrappers.days )
                        plugin.target
                                .find( plugin.settings.wrappers.days )[plugin.settings.value_setter]($days)
                    if ( undefined != plugin.settings.wrappers.hours )
                        plugin.target
                                .find( plugin.settings.wrappers.hours )[plugin.settings.value_setter]($hours)
                    if ( undefined != plugin.settings.wrappers.minutes )
                        plugin.target
                                .find( plugin.settings.wrappers.minutes )[plugin.settings.value_setter]($minutes)
                    if ( undefined != plugin.settings.wrappers.seconds )
                        plugin.target
                                .find( plugin.settings.wrappers.seconds )[plugin.settings.value_setter]($seconds)
                } else {
                    plugin.target[plugin.settings.value_setter](value)
                }
            }
